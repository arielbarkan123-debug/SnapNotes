...
export default function HomeworkResultsPage() {
  ...
  const autoStartRetries = useRef(0)
  const MAX_AUTO_START_RETRIES = 2

  const handleSendMessage = useCallback(async (message: string, explanationStyle?: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/homework/sessions/${sessionId}/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message, enableDiagrams: visualPrefs.autoGenerateDiagrams, explanationStyle }),
  ...
  }, [isChatLoading, sessionId, toast, trackFeature, visualPrefs.autoGenerateDiagrams])
