...
export default function SettingsPage() {
  ...
  const {
    ...
    setComplexity,
    setAnimationSpeed,
    savePreferences: saveVisualPreferences,
  } = useVisuals()
        ...
        <section className="mb-6">
          ...
          >
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-white">{t('visuals.enabled')}</h4>
                  <p className="text-sm text-gray-500 dark:text-gray-400">{t('visuals.enabledDesc')}</p>
                </div>
                <button
                  onClick={() => {
                    toggleVisuals()
                  ...
                  className={`
                    ...
                    ${visualPreferences.visualsEnabled
                      ? 'bg-violet-600'
                ...
                >
                  <span
                    className={`
                      ...
                      ${visualPreferences.visualsEnabled ? 'translate-x-5' : 'translate-x-0'}
                    `}
                  />
                </button>
              </div>

              {/* Auto-Generate Diagrams Toggle */}
              <div className="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-white">{t('visuals.autoGenerate')}</h4>
                  <p className="text-sm text-gray-500 dark:text-gray-400">{t('visuals.autoGenerateDesc')}</p>
                </div>
                <button
                  onClick={() => {
                    toggleAutoGenerate()
                  ...
                  disabled={!visualPreferences.visualsEnabled}
                  className={`
                    relative h-6 w-11 rounded-full transition-colors
                    ${!visualPreferences.visualsEnabled ? 'opacity-50 cursor-not-allowed' : ''}
                    ${visualPreferences.autoGenerateDiagrams && visualPreferences.visualsEnabled
                      ? 'bg-violet-600'
                      : 'bg-gray-300 dark:bg-gray-600'
                    }
                  `}
                >
                  <span
                    className={`
                      absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform
                      ${visualPreferences.autoGenerateDiagrams && visualPreferences.visualsEnabled ? 'translate-x-5' : 'translate-x-0'}
                    `}
                  />
                </button>
              </div>
                  ...
                  {(['slow', 'normal', 'fast', 'instant'] as const).map((speed) => (
                    <button
                      ...
                      className={`
                        flex flex-col items-center gap-1 rounded-xl border-2 p-3 transition-all
                        ${!visualPreferences.visualsEnabled ? 'opacity-50 cursor-not-allowed' : ''}
                        ${visualPreferences.animationSpeed === speed
                          ? 'border-violet-500 bg-violet-50 dark:border-violet-400 dark:bg-violet-500/10'
                          : 'border-gray-200 bg-white hover:border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:hover:border-gray-600'
                        }
                      `}
                    >
                      ...
                      <span className="text-xs font-medium text-gray-700 dark:text-gray-300">
                        {t(`visuals.animation${speed.charAt(0).toUpperCase() + speed.slice(1)}`)}
                      </span>
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {t('visuals.complexity')}
                </label>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">{t('visuals.complexityDesc')}</p>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {([
                    { id: 'elementary', icon: '🎨' },
                    { id: 'middle_school', icon: '📐' },
                    { id: 'high_school', icon: '📊' },
                    { id: 'advanced', icon: '🧮' },
                    ...
                    <button
                      key={id}
                      onClick={() => {
                        setComplexity(id as VisualComplexity)
                        setHasChanges(true)
                      }}
                      disabled={!visualPreferences.visualsEnabled}
                      className={`
                        flex flex-col items-center gap-1 rounded-xl border-2 p-3 transition-all
                        ${!visualPreferences.visualsEnabled ? 'opacity-50 cursor-not-allowed' : ''}
                        ${visualPreferences.complexity === id
                          ? 'border-violet-500 bg-violet-50 dark:border-violet-400 dark:bg-violet-500/10'
                          : 'border-gray-200 bg-white hover:border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:hover:border-gray-600'
                        }
                      `}
                    >
                      ...
                      <span className="text-xs font-medium text-gray-700 dark:text-gray-300 text-center">
                        {t(`visuals.complexity${id.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('')}`)}
                      </span>
                    </button>
                  ))}
                </div>
