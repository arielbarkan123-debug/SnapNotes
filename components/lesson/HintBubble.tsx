      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-xs font-semibold uppercase tracking-wide mb-1 ${getTextColor()} opacity-70`}>
            {hint.type === 'tip' && 'Tip'}
            {hint.type === 'reminder' && 'Reminder'}
            {hint.type === 'simplification' && 'Try This'}
            {hint.type === 'example' && 'Example'}
          </p>
          ...
          {showFeedback && (
            <div className="mt-3 flex items-center gap-2">
              <span className={`text-xs ${getTextColor()} opacity-70`}>Was this helpful?</span>
              <button
  ...
  return (
    ...
    >
      ...
      {hintsRemaining !== undefined && hintsRemaining > 0 && (
        <span className="ml-1 px-1.5 py-0.5 bg-yellow-200 rounded-full text-xs">
          {hintsRemaining}
        </span>
      )}
    </button>
