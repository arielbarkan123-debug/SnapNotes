...
/** Diagram generation mode: off = no diagram, quick = TikZ only (~8s), accurate = full routing (~20s) */
export type DiagramMode = 'off' | 'quick' | 'accurate'

export interface VisualPreferences {
  /** Whether to show visual diagrams by default */
  visualsEnabled: boolean
  /** Whether AI should auto-generate diagrams when explaining (derived from diagramMode for backward compat) */
  autoGenerateDiagrams: boolean
  /** Diagram generation mode: off / quick / accurate */
...
interface VisualsContextValue {
  ...
  toggleVisuals: () => void
  /** Toggle auto-generate diagrams (cycles: off → quick → accurate → off) */
  toggleAutoGenerate: () => void
  /** Set diagram mode directly */
...
const DEFAULT_PREFERENCES: VisualPreferences = {
  ...
  autoGenerateDiagrams: true, // derived from diagramMode (backward compat)
  diagramMode: 'quick', // Quick mode by default — fast TikZ pipeline
...
export function VisualsProvider({ children }: VisualsProviderProps) {
        ...
        if (typeof window !== 'undefined') {
          try {
            ...
            if (stored) {
              ...
              // Backward compat: migrate autoGenerateDiagrams → diagramMode
              if (!parsed.diagramMode && parsed.autoGenerateDiagrams !== undefined) {
                parsed.diagramMode = parsed.autoGenerateDiagrams ? 'quick' : 'off'
              }
              // Keep autoGenerateDiagrams in sync with diagramMode
              if (parsed.diagramMode) {
                parsed.autoGenerateDiagrams = parsed.diagramMode !== 'off'
              }
  ...
  const setDiagramMode = useCallback((mode: DiagramMode) => {
    setPreferences(prev => {
      const updated = { ...prev, diagramMode: mode, autoGenerateDiagrams: mode !== 'off' }
      saveToLocalStorage(updated)
      return updated
    })
  }, [])
  ...
  const toggleAutoGenerate = useCallback(() => {
    setPreferences(prev => {
      const cycle: DiagramMode[] = ['off', 'quick', 'accurate']
      const nextIdx = (cycle.indexOf(prev.diagramMode) + 1) % cycle.length
      const nextMode = cycle[nextIdx]
      const updated = { ...prev, diagramMode: nextMode, autoGenerateDiagrams: nextMode !== 'off' }
      saveToLocalStorage(updated)
      return updated
    })
  }, [])
